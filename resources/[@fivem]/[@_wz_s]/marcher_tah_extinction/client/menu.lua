...
AddEventHandler("esx_leboncoin:cb", function(ownedVehicles, availableVehicles)
    ...
    RMenu.Add(cat, sub("main"), RageUI.CreateMenu("WZ Market","~g~WZ ~w~Market"))
    RMenu:Get(cat, sub("main")).Closed = function()end


    RMenu.Add(cat, sub("categoryOffers"), RageUI.CreateSubMenu(RMenu:Get(cat, sub("main")), "WZ Market", "~y~Parcourir les offres"))
    RMenu:Get(cat, sub("categoryOffers")).Closed = function()end

    RMenu.Add(cat, sub("sell"), RageUI.CreateSubMenu(RMenu:Get(cat, sub("main")), "WZ Market", "~y~Mettre en vente"))
    RMenu:Get(cat, sub("sell")).Closed = function()end

    RMenu.Add(cat, sub("sellVehicle"), RageUI.CreateSubMenu(RMenu:Get(cat, sub("sell")), "WZ Market", "~y~Mettre en vente"))
        ...
        while isMenuOpened do
            ...
            RageUI.IsVisible(RMenu:Get(cat, sub("main")),true,true,true,function()
                ...
                if #ownedVehicles > 0 then
                    RageUI.ButtonWithStyle(("%s→ ~s~Mettre un véhicule en vente ~s~(~y~%i~s~)"):format(rgb(), #ownedVehicles), "~y~Description~s~: Vous permets de mettre un de vos véhicule en vente sur WZ Market", {}, true, function(_,_,s)
                    end, RMenu:Get(cat, sub("sell")))
                else
                    RageUI.ButtonWithStyle("~g~Aucun item disponible", nil, {}, true)
                end
                RageUI.Separator("↓ ~g~Items en vente ~s~↓")
